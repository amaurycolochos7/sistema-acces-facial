...
export default function ExitReasonsPage() {
  ...
  const [form, setForm] = useState({ name: '', category: 'TEMPORARY', icon: '📋', sortOrder: 0, isDefault: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowForm(false);
    setEditingId(null);
    setForm({ name: '', category: 'TEMPORARY', icon: '📋', sortOrder: 0, isDefault: false });
    fetchReasons();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setForm({ name: '', category: 'TEMPORARY', icon: '📋', sortOrder: 0, isDefault: false }); setEditingId(null); setShowForm(true); }}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-center outline-none focus:ring-2 focus:ring-blue-500/20" />
                </div>
                <div>
